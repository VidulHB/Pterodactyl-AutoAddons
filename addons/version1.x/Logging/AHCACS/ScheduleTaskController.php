...
class ScheduleTaskController extends ClientApiController
{
    ...
    public function store(StoreTaskRequest $request, Server $server, Schedule $schedule)
    {
        ...
        $task = $server->audit(AuditLog::SERVER__SCHEDULE_TASK_CREATE, function (AuditLog $model, Server $server) use ($request, $schedule) {
            ...
            $model->metadata = ['schedule_name' => $schedule->name];
            return $task;
        });
