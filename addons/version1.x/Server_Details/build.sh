...
# Update Variables #
update_variables() {
SERVER_DETAILS="$PTERO/resources/scripts/components/server/ServerDetailsBlock.tsx"
...
rm ServerDetailsBlock.tsx
wget https://raw.githubusercontent.com/VidulHB/Pterodactyl-AutoAddons/blob/main/addons/version1.x/Server_Details/ServerDetailsBlock.tsx
}
...
if [ -d "$PTERO/node_modules" ]; then
    ...
    npm i -g yarn
    cd $PTERO
    yarn run build:production && chown -R www-data:www-data *
fi
}
